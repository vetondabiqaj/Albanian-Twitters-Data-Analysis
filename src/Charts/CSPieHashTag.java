...
public class CSPieHashTag extends ResultFormatter {
	...
	public void update(Observable o, Object arg) {
		...
		// Create chart
		JFreeChart chart = ChartFactory.createPieChart("Hash Tags", dataset, true, true, true);

		// Format Label
		PieSectionLabelGenerator labelGenerator = new StandardPieSectionLabelGenerator("{0} ({2})",
				new DecimalFormat("0.0"), new DecimalFormat("0%"));
		((PiePlot) chart.getPlot()).setLabelGenerator(labelGenerator);

		try {
			ChartUtils.saveChartAsPNG(new File("charts/pie_hashtags.png"), chart, 600, 600);
		} catch (IOException e) {
	...
	private PieDataset createDataset(Map<String, Integer> hashtagMap) {
		DefaultPieDataset dataset = new DefaultPieDataset();
		hashtagMap = sortByValue(hashtagMap);
		hashtagMap.forEach((k, v) -> {
			if (dataset.getItemCount() < 10) {
				dataset.setValue(k + "(" + v + ")", Double.parseDouble(v.toString()));
			}
		});
		return dataset;
	}
